...
var ErrorFormater, Joi, MagicRoute, Router, _, applyMiddleware, applyRoute, express, init, schemaValidMiddleware;
...
schemaValidMiddleware = function(schema, where, errorCode) {
  if (errorCode == null) {
    errorCode = '-10001';
  }
  return function(req, res, next) {
    ...
    validRet = Joi.validate(req[where], schema(req, Joi), {
      allowUnknown: true
    });
  ...
  for (index = i = 0, len = ref.length; i < len; index = ++i) {
    ...
    if (index === 0) {
      if (_.isFunction(routeConfig.schema)) {
        applyMiddleware(url, routeConfig.method, schemaValidMiddleware(routeConfig.schema, routeConfig.where, routeConfig.errorCode));
      }
    }
